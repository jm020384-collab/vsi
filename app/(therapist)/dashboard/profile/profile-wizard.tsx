  ...
  if (state?.ok) {
    return (
      <div className="mx-auto w-full max-w-[560px] py-16 text-center">
        ...
        >
          {state.status === "APPROVED" ? "Зміни збережено" : "Анкету відправлено на перевірку"}
        </h1>
        <p className={cn("mt-3 text-[15px] leading-relaxed", ink.muted)}>
          {state.status === "APPROVED"
            ? "Ваш профіль уже опубліковано, тож правки видно відвідувачам одразу — повторна перевірка не потрібна."
            : "Ми звіримо дані вручну — зазвичай це займає до двох робочих днів. Свій простір ви вже можете переглянути: він виглядає так само, як його побачать відвідувачі після публікації."}
        </p>
